...
- **A pause control ships with it.** WCAG 2.2.2 "Pause, Stop, Hide" is **level A**, and it applies squarely: motion that starts automatically, runs past five seconds, and sits in parallel with other content must have a mechanism to stop it. The control is persistently visible — not revealed on hover — keyboard reachable, and carries a real accessible name. Its state persists for the visit.
